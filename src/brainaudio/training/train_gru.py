...
class BrainToTextDecoder_Trainer:
    ...
    def train(self):
        ...
        wandb.init(project="nejm-brain-to-text" , 
                entity="lionelhu926-ucla", config=dict(self.args), name=args['modelName'])
        # Set model to train mode (specificially to make sure dropout layers are engaged)
        self.model.train()

        wandb.watch(self.model, log="all")  # Logs gradients, parameters, and gradients histograms
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if i % self.args['batches_per_train_log'] == 0:
                ...
                train_log_dict = {
                    "train_ctc_loss": f"{(loss.detach().item()):.2f}",
            ...
            if i % self.args['batches_per_val_step'] == 0 or i == ((self.args['num_training_batches'] - 1)):
                ...
                self.logger.info(f'Val batch {i}: ' +
                 ...
                 val_log_dict = {
                    "val_ctc_loss": f"{val_metrics["avg_loss"]:.4f}",
                    "per": per_array[0],
                    "learning_rate": current_lr, 
                    "val_time": {val_step_duration:.3f}
                }
