...
class GRU(BaseTimeMaskedModel):
    """Unified GRU-based speech encoder.
    ...
    def __init__(
        ...
        *,
        ...
        self.day_weights = nn.ParameterList(
            [nn.Parameter(torch.eye(neural_dim)) for _ in range(n_day_weights)]
        )
        self.day_biases = nn.ParameterList(
            [nn.Parameter(torch.zeros(1, neural_dim)) for _ in range(n_day_weights)]
        )

        self.day_layer_dropout = nn.Dropout(input_dropout)

        input_size = neural_dim * kernelLen if kernelLen > 0 else neural_dim
        ...
        self.out = nn.Linear(rnn_out_dim, n_classes + 1)
        nn.init.xavier_uniform_(self.out.weight)

        h0_layers = layer_dim * 2 if bidirectional else layer_dim
        self.h0 = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(h0_layers, 1, hidden_dim)))
    ...
    def forward(self, x, x_len, day_idx):
        ...
        states = self.h0.expand(-1, x.shape[0], self.hidden_dim).contiguous()
