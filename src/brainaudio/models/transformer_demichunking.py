...
from brainaudio.models.utils.demi_chuking_utils import ChunkConfig, ChunkConfigSampler, create_dynamic_chunk_mask
...
class TransformerModel(BaseTimeMaskedModel):
    ...
    def _setup_chunked_attention(self, config_dict) -> None:
        ...
        """Set up demi-chunk config sampler and eval config from training config dict."""
        ...
        # chunk_size is null → non-causal: entire trial is one chunk (full bidirectional)
        # chunk_size>=1, context_sec=null → causal with full left context (context_chunks=None)
        # chunk_size>1, context_sec=X   → causal with limited left context
        if eval_chunk_size is None:
            self._eval_config = ChunkConfig(chunk_size=None, context_chunks=None)
        else:
            if eval_context_secs is not None:
                total_context_timesteps = eval_context_secs / timestep_duration_sec
