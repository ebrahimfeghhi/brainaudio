...
class GRU_24(BaseTimeMaskedModel):
    ...
    def __init__(
        ...
        *,
        ...
        self.dayWeights = nn.Parameter(torch.randn(n_day_weights, self.neural_dim, self.neural_dim))
        self.dayBias = nn.Parameter(torch.zeros(n_day_weights, 1, self.neural_dim))
    ...
    def forward(self, neuralInput: torch.Tensor, X_len: torch.Tensor, dayIdx: torch.Tensor
        ...
        # --- Day‑specific (or shared) affine transform ---
        idx = torch.zeros_like(dayIdx) if self.shared_input else dayIdx
        dayWeights = torch.index_select(self.dayWeights, 0, idx)
