    ...
    for causal_mode in [True, False]:
        ...
        if dataset == "b2t_25":
            ...
            if transformer_mode: 
                ...
                if causal_mode: 
                    ...
                    logits_file_name = [f"logits_{partition}_chunk:1_context:20.npz"] * len(seeds_list)
                    nbest_save_path_arr = [f"time_masked_transformer_25/seed_{i}_unidir_{partition}_transformer" for i in seeds_list]

                else:

                    folder_name = [f"neurips_b2t_25_bidirectional_transformer_v4_seed_{i}" for i in seeds_list]
                    logits_file_name = [f"logits_{partition}_chunk:full_context:full.npz"] * len(seeds_list)
                    nbest_save_path_arr = [f"time_masked_transformer_25/seed_{i}_bidir_{partition}_transformer" for i in seeds_list]
        ...
        if dataset == "b2t_24":
            ...
            if transformer_mode: 
                ...
                if causal_mode: 
                    ...
                    logits_file_name = [f"logits_{partition}_chunk:1_context:7.5.npz"] * len(seeds_list)
                    nbest_save_path_arr = [f"time_masked_transformer_24/seed_{i}_unidir_{partition}_transformer" for i in seeds_list]

                else:

                    folder_name = [f"neurips_b2t_24_bidir_transformer_seed_{i}" for i in seeds_list]
                    logits_file_name = [f"logits_{partition}_chunk:full_context:full.npz"] * len(seeds_list)
                    nbest_save_path_arr = [f"time_masked_transformer_24/seed_{i}_bidir_{partition}_transformer" for i in seeds_list]
        ...
        for fn, lm, nb in zip(folder_name, logits_file_name, nbest_save_path_arr):
            ...
            if dataset == "b2t_25":
                ...
                nbest_augmented_save_path = f"{save_path}{nb}_augmented.pkl"
